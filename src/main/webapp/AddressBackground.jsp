			...
			<form action="insertServiceTable.action" class="layui-form"
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<input type="text" lay-verify="username" id="recipName" name="recipName" autocomplete="off"
							class="layui-input">
					</div>
				</div>
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<input type="text" id="recipMobile" name="recipMobile" autocomplete="off"
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<input type="text" id="recipPhone" name="recipPhone" autocomplete="off"
							class="layui-input">
					</div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						...
						<input type="hidden" id="recipAddress" name="recipAddress" autocomplete="off" class="layui-input">
					</div>
				</div>
				<div class="layui-form-item">
					<label class="layui-form-label">详细地址：</label>
					<div class="layui-input-block">
						<input type="text" id="recipDetailed" name="recipDetailed" autocomplete="off"
							class="layui-input">
					</div>
				</div>
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<select name="recipType" id="recipType"  lay-filter="aihao">
							<option value=""></option>
  ...
  <div class="layui-btn-container">
	...
	<script>
		...
		layui.use([ 'form', 'table','element', 'layedit', 'jquery', 'layer' ],function() {
						...
						//新增方法
						$("#save").click(function(){
							var recipName = "recipName="+$("#recipName").val();
							var recipMobile = "recipMobile="+$("#recipMobile").val();
							var recipPhone = "recipPhone="+$("#recipPhone").val();
							var recipAddress = "recipAddress="+$("#recipAddress").val();
							var recipDetailed = "recipDetailed="+$("#recipDetailed").val();
							var recipType = "recipType="+$("#recipType").val();
							var senderName = "senderName="+$("#senderName").val();
							var userInfoPhone = "userInfoPhone="+$("#userInfoPhone").val();
							$.ajax({
								type:"post",
								url:"http://localhost:8080/stoMaven/insertorderInforMation_gsp_zsh.action",
								data:recipName+"&"+recipMobile+"&"+recipPhone+"&"+recipAddress+"&"+recipDetailed+"&"+recipType+"&"+senderName+"&"+userInfoPhone,
								dataType:"text",
								success : function(msg){
									window.location.href = "AddressBackground.jsp";
								}
							});
						})
			...
			layui.use('table', function() {
				...
				table.render({
					...
					url : 'selectorderInforMation_gsp_zshAllpage.action',
					toolbar : '#toolbarDemo',
					title : '收件订单表',
					cols : [ [ {
						type : 'numbers'
			...
			table.on('tool(SeviceTable_xwc)', function(obj) {
				var data = obj.data;
				if (obj.event === 'edit') {
					$.post('updateorderInforMation_gsp_zsh.action',data);
					layer.msg("修改成功！");
				} else if (obj.event === 'del') {
					 layer.confirm('真的删除行么', function(index){
					        obj.del();
					        layer.close(index);
					        $.post('deleteorderInforMation_gsp_zsh.action','recipientId='+ data.recipientId, function(msg) {
							});
						layer.msg("删除成功！");
					 });
				}
